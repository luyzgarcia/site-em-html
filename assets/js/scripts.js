...
$(document).ready(function() {
	...
	function togglePlanta(ocultar, self) {
		//$(self).children('.planta').text('aaa');
		if(typeof(ocultar) === 'undefined') ocultar = true;
		if(ocultar) {
			$($(self).children('.planta')).toggle( 'slow',function () {
				/*if ($(this).is(":visible")) {
					//Adiciona o css 
					$('#bt_verplanta h4').text('Esconder planta');
					$( "#bt_verplanta" ).addClass( "ver_planta_ativo" );
			    } else {
			        //Remove o css
